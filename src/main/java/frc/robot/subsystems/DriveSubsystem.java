...
public class DriveSubsystem extends SubsystemBase {
  ...
  public void setModuleStates(SwerveModuleState[] desiredStates) {
    for (int i = 0; i < desiredStates.length; i++) {
      double speed = desiredStates[i].speedMetersPerSecond;
      double maxSpeed = DriveConstants.kMaxSpeedMetersPerSecond;
      
      // Apply your custom logic to limit the speed
      double limitedSpeed = Math.min(Math.max(speed, -maxSpeed), maxSpeed);
      
      // Update the speed in the SwerveModuleState
      desiredStates[i] = new SwerveModuleState(
          limitedSpeed, desiredStates[i].angle);
  }
